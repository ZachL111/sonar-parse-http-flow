...
`sonar-parse-http-flow` is centered on parsers work in Ruby. The technical target is: Implement a Ruby parsers project for http state machine modeling, using transition tables and invalid-transition tests.
...
The score is deterministic. The current threshold is 176. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
